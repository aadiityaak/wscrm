...
This is a **Laravel 12 + Vue 3 + Inertia.js** application with the following setup:
- **Backend**: Laravel 12 with PHP 8.2+
- **Frontend**: Vue 3 with TypeScript, TailwindCSS 4.x, and Reka UI components
- **Data Flow**: Inertia.js for SPA-like experience without API
...
- `composer run dev:ssr` - Development with SSR support (includes Pail logs and Inertia SSR)
...
- `npm run lint` - Frontend linting and fixing
...
- `resources/js/pages/` - Vue page components (corresponds to routes)
- `resources/js/components/` - Reusable Vue components
- `resources/js/components/ui/` - Reka UI component library
- `resources/js/layouts/` - Layout components for different sections
- `resources/js/composables/` - Vue composables for shared logic
- `routes/` - Route definitions split by feature (web.php, auth.php, settings.php)

### Inertia.js Data Flow
- Controllers return `Inertia::render('PageName', $data)` instead of JSON API responses
- Vue pages receive data as props automatically
- Form submissions use Inertia's form helpers for SPA-like behavior
- Page components are auto-resolved from `resources/js/pages/` directory

### Authentication System
- Complete authentication flow with email verification
- Middleware-protected routes in `routes/web.php`
- User profile and password management in Settings section
- Uses Laravel's built-in authentication with Inertia adaptations

### UI Components
- **Reka UI**: Primary component library (headless UI for Vue)
...
### Creating New Pages
1. Add route in appropriate route file (`routes/web.php`, `routes/auth.php`, etc.)
2. Create controller method returning `Inertia::render('PageName', $data)`
3. Create Vue page in `resources/js/pages/PageName.vue`
4. Use existing layouts from `resources/js/layouts/`

### Form Handling
- Use Inertia's `useForm()` composable for reactive forms
- Laravel validation rules in Request classes (`app/Http/Requests/`)
- Error handling automatically passed to frontend

### Component Development
- Follow existing UI component patterns in `resources/js/components/ui/`
- Use Reka UI primitives for accessibility
- Implement consistent styling with TailwindCSS utilities
